...
import gleam/result
import gleam/string
import gleam/gleeunit/gleam_panic.{type GleamPanic}
...
fn format_gleam_error(
  ...
  let location = grey(error.file |> string.drop_start("src/" |> string.length) <> ":" <> int.to_string(error.line))
...
@external(javascript, "./gleam_gleeunit_gleeunit_ffi.mjs", "read_file")
